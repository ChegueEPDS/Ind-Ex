...
import { NewzoneComponent } from '../../zone/newzone/newzone.component';
...
@Component({
  ...
  styleUrl: './sitedetail.component.scss',
  providers: [DatePipe]
})
export class SitedetailComponent implements OnInit {
  ...
  fetchZones(): void {
    ...
    // 🔹 1. Zónák lekérése az adott Site ID alapján
    this.http.get<any[]>(`${environment.apiUrl}/api/zones?siteId=${this.siteId}`).subscribe({
        next: (zonesData) => {
            this.zones = zonesData.filter(zone => zone.Company === userCompany);
            ...
            // 🔹 2. Equipmentek lekérése minden egyes zónához
            this.zones.forEach(zone => {
                this.http.get<any[]>(`${environment.apiUrl}/api/exreg?Zone=${zone._id}`).subscribe({
                    next: (equipments) => {
                        zone.equipmentCount = equipments.length; // Hozzáadjuk az equipmentek számát
                    },
                    error: (err) => {
                        console.error(`Hiba a(z) ${zone.Name} equipment-ek lekérésekor:`, err);
                        zone.equipmentCount = 0; // Ha hiba van, akkor 0 legyen az alapértelmezett érték
                    }
                });
            });
